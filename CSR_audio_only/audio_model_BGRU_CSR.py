...
class ASR(nn.Module):
    def __init__(self, mode, inputDim=256, hiddenDim=512, nClasses=100, frameLen=60, every_frame=True):
        ...
        self.fronted2D = nn.Sequential(
                nn.Conv2d(1, 64, kernel_size=(3, 3), stride=(1, 2), padding=(1, 1), bias=False),
                nn.BatchNorm2d(64),
                nn.ReLU(True),
...
def audiosr(mode, inputDim=256, hiddenDim=512, nClasses=500, frameLen=25, every_frame=True):
        model = ASR(mode, inputDim=inputDim, hiddenDim=hiddenDim, nClasses=nClasses, frameLen=frameLen, every_frame=every_frame)
        return model
