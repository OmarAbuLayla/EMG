...
def test_adam(args, use_gpu):
    ...
    save_path = os.path.join(os.getcwd(), "emg_only_" + args.mode + "_every_frame")
    os.makedirs(save_path, exist_ok=True)

    # logging info
    filename = os.path.join(save_path, f'{args.mode}_emg_{args.lr}.txt')
    logger_name = "mylog"
    ...
    elif args.mode == 'transformer':   # <-- ADD THIS
        optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=0.)  # standard Adam
    else:
