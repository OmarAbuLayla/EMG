...
def train_test(model, dset_loaders, criterion, epoch, phase, optimizer, args, logger, use_gpu, save_path, debug_overfit=False):
    ...
    # If debugging overfitting: only use tiny batch
    tiny_inputs, tiny_targets = None, None
    if debug_overfit:
        tiny_loader = iter(dset_loaders['train'])
        tiny_inputs, tiny_targets = next(tiny_loader)
        tiny_inputs = tiny_inputs[:10]
        tiny_targets = tiny_targets[:10]
        logger.info(f"[DEBUG] Overfitting mode ON. Using {tiny_inputs.size(0)} samples only.")

    dataset_phase = 'train' if (phase == 'train' and not debug_overfit) else phase

    with torch.set_grad_enabled(phase == 'train'):
        for batch_idx, (inputs, targets) in enumerate(dset_loaders[dataset_phase]):
            if debug_overfit:
                inputs, targets = tiny_inputs, tiny_targets
            ...
            # === DEBUGGING INPUTS ===
            if batch_idx == 0:
                print(f"[DEBUG] Input stats: min={inputs.min().item():.4f}, max={inputs.max().item():.4f}, mean={inputs.mean().item():.4f}")
                print(f"[DEBUG] Target stats: min={targets.min().item()}, max={targets.max().item()}")
            ...
            if batch_idx == 0:
                print(f"[DEBUG] Preds  : {preds.cpu().numpy()}")
                print(f"[DEBUG] Targets: {targets.cpu().numpy()}")
            ...
            if debug_overfit:
                break  # Only first batch in overfit mode

    epoch_loss = running_loss / running_all
    epoch_acc = running_corrects / running_all
    logger.info(f"{phase} Epoch:\t{epoch}\tLoss: {epoch_loss:.4f}\tAcc:{epoch_acc:.4f}")

    if phase == 'train' and not debug_overfit:
...
def test_adam(args, use_gpu):
    ...
    # === DEBUG OVERFITTING CHECK ===
    debug_overfit = args.debug_overfit

    if args.test:
        train_test(model, dset_loaders, criterion, 0, 'val', optimizer, args, logger, use_gpu, save_path, debug_overfit)
        train_test(model, dset_loaders, criterion, 0, 'test', optimizer, args, logger, use_gpu, save_path, debug_overfit)
        return

    for epoch in range(0,args.epochs):
        scheduler.step(epoch)
        model = train_test(model, dset_loaders, criterion, epoch, 'train', optimizer, args, logger, use_gpu, save_path, debug_overfit)
        train_test(model, dset_loaders, criterion, epoch, 'val', optimizer, args, logger, use_gpu, save_path, debug_overfit)
...
def main():
    ...
    parser.add_argument('--debug-overfit', default=False, action='store_true', help='debug mode: overfit tiny batch')
